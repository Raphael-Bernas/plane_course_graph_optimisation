...
function animateGraphTraversal(file::String, output_path::String)
    ...
    anim = @animate for step in 1:length(path)
        ...
        graphplot(A,
                  markersize = 4,  # Increased size of vertex circles
                  node_weights = 1:n,
                  ...
                  size = (800, 800),    # Increase the size of the plot
                  xlims = (minimum(positions[:, 1]) - 1, maximum(positions[:, 1]) + 1),  # Set x-axis limits
                  ylims = (minimum(positions[:, 2]) - 1, maximum(positions[:, 2]) + 1)   # Set y-axis limits
                  )
    end
...
animateGraphTraversal("/home/bernas/VSC/JULIA/SOD321/project/Instances/instance_20_1.txt", "graph_traversal.gif")